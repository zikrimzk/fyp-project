...
class EvaluationController extends Controller
{
    ...
    /* Evaluation Student [Evaluator] - Route | Last Check: 17-08-2025 */
    public function evaluationStudent($evaluationID, $mode)
    {
        try {
            ...
            /* LOAD ALL ROLE FORM SIGNATURE KEY  */
            $allRoleSignature = FormField::where('af_id', $actForm->id)
                ->where('ff_category', 6)
                ->select('ff_label', 'ff_signature_key')
                ->get();
            ...
            return view('staff.evaluation.evaluation-student', [
                ...
                'allRoleSignature' => $allRoleSignature,
                'data' => $data,
                'mode' => $mode,
                'page' => $page,
                'link' => $link,
    ...
    /* Individual Signature Pad [Evaluator] - Route | Last Check: 17-08-2025 */
    public function evaluationSignaturePad($signature_key, $evaluationID, $mode)
    {
        try {

            /* DECRYPT IDs */
            $evaluationID = decrypt($evaluationID);
            $signature_key = decrypt($signature_key);
            ...
            if (!$form) {
                return back()->with('error', 'Error occurred: Activity form not found.');
            }

            /* LOAD EXAMINER SIGNATURE  */
            $examinerSign = FormField::where('af_id', $form->id)
                ->where('ff_signature_role', 8)
                ->select('ff_signature_key')
                ->get();

            return view('staff.evaluation.evaluation-signature-pad', [
                'title' => 'Evaluation Signature Pad',
                'student' => $student,
                'evaluationID' => $evaluationID,
    ...
    /* Get Evaluation Form Signature (Method: Input) - Function | Last Check: 17-08-2025 */
    public function getEvaluationFormSignature(Request $req)
    {
        try {
            ...
            /* LOAD FORM FIELD DATA */
            $formfields = FormField::where('af_id', $form->id)
                ->whereIn('ff_category', [2, 6])
                ->where(function ($q) use ($sigkey) {
                    $q->where('ff_category', '!=', 6)
                        ->orWhere(function ($q2) use ($sigkey) {
                            $q2->where('ff_category', 6)
                                ->where('ff_signature_key', $sigkey);
                        });
                })
                ->orderBy('ff_order')
                ->get();
    ...
    /* Evaluation Signature Submission [Evaluator] - Function | Last Checked : 17-08-2025 */
    public function submitSignatureData(Request $req, $evaluationID, $mode)
    {
        try {
            /* DECRYPT PROCESS */
            $evaluationID = decrypt($evaluationID);
            ...
            $nomination = Nomination::where('student_id', $evaluation->student_id)
                ...
                ->whereHas('evaluators', function ($query) {
                    $query->where('eva_status', 3);
                })
                ->first();

            if (!$nomination) {
                return back()->with('error', 'Active nomination record not found. Operation could not be processed. Please contact administrator for further assistance.');
            }
    ...
    public function correctionFinalOverview(Request $req)
    {
        try {
            ...
            /* RETURN VIEW */
            return view('staff.evaluation.correction-final-overview', [
    ...
    public function updateFinalCorrection(Request $req, $id)
    {
        ...
        try {
            ...
            if ($update) {
                $this->restoreSubmission($student, $activity, $validated['ac_duedate_up']);
            }
    ...
    /* Correction Approval [Staff] - Route */
    public function correctionApproval(Request $req)
    {
        try {
            ...
            return view('staff.evaluation.correction-approval', [
    ...
    /* Correction Approval - [Examiners/Panels] - Route */
        ...
        try {
            ...
            if ($req->ajax()) {
                ...
                $table->addColumn('action', function ($row) {
                    ...
                    /* GATHER ALL EXAMINERS THAT BEING NOMINATED */
                    ...
                    /* IF NOT ONE OF THE EXAMINERS */
                    if (! in_array($userId, $examinerIds, true)) {
                        return '<span class="fst-italic text-muted">No action required</span>';
                    }

                    /* IF EXAMINER AND FINDING MY SLOT */
                    $myIndex = array_search($userId, $examinerIds, true);

                    /* GET ALL SIGNATURE KEYS FOR THE EXAMINER */
                    ...
                    /* PENDING EXAMINER GET THE BUTTON */
            ...
            return view('staff.correction.examiner-correction-approval', [
                'title' => 'Examiner/Panel - Correction Approval',
                'studs' => Student::all(),
                'progs' => Programme::all(),
                'facs' => Faculty::all(),
                'sems' => Semester::all(),
    ...
    /* Student Correction Approval [Staff]  - Function | Email : Yes With Works */
    public function studentActivityCorrectionApproval(Request $request, $actCorrID, $option)
    {
        ...
        try {
            ...
            if ($option == 1) {
                ...
                [$role, $status] = $sc->determineApprovalRoleStatus($supervision, $evaluator, $authUser->staff_role, 2);

                /* MERGE AND HANDLE FORM */
                $sc->mergeStudentSubmission($actID, $student, $semester, $request->input('signatureData'), $role, $authUser, $status, 2, $evaluatorIndex, null, $activityCorrection);
                ...
                $sc->handleSignatureApprovalStatus($student, null, $updatedCorrection, $activity, $afID, $role, $hasCoSv, $updatedSignatureData, null, null, 2);
                ...
                [$role, $status] = $sc->determineRejectionRoleStatus($supervision, $evaluator, $authUser->staff_role, 2);
    ...
    /* Extract Evaluation Status [Staff] - Function */
    public function extractEvaluationStatus($evaluationMetaData)
    {
        ...
        /* LIST OF POSSIBLE VALUE KEYS FOR PROGRESS */
        $progressSynonyms = [
        ...
        /* LIST OF POSSIBLE VALUE KEYS FOR MOCK */
        $mockVivaSynonyms = [
            'mock viva',
            'mv',
            'mock',
        ...
        /* FIND THE STATUS LIKE FIELD ANOTHER WAY */
        if (!$statusValue) {
        ...
        /* MATCH WITH THE LIST OF SYNONYMS FOR VALUE */
        if ($statusValue) {
